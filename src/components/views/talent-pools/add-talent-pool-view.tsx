...
import { ADD_TALENT_POOL_MUTATION } from '@/graphql-operations/mutations/signup-candidate'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    createEntity({
      ...
      .then(async (res) => {
        ...
        client
          .refetchQueries({
            include: ['GET_HUB_POOLS', 'GET_ADD_CANDIDATE_DROPDOWNS'],
          })
          .then(() => {
            Alert({ type: 'success', message: 'Talent Pool created successfully' }).then()
          })
      })
      .catch((err) => {
        console.error(err)
    ...
    <form onSubmit={handleSubmit} className="mb-2 h-full max-h-full p-2">
      ...
      <div>
        <Button className="w-full " color="primary" type="submit">
          {onSubmitLoading ? <Loader size="h-4 w-4" fullScreen={false} /> : 'Add Talent Pool'}
        </Button>
      </div>
    </form>
  )
